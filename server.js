		...
		if (user) {
			...
			bcrypt.genSalt(10, (err, salt) => {
				bcrypt.hash(user.password, salt, (err, hash) => {
					user.password = hash;
					user.save().then((user) => {
						jwt.sign({ id: user._id, role: "teacher", name: user.name }, "secretkey", (err, token) => {
							res.send({
								token,
								user: {
									id: user._id,
...
app.post("/publishQuiz", auth, (req, res) => {
	...
	Quiz.findOneAndUpdate(
		...
		.then((quiz) => {
			emailInvites(quiz.quizInvites.contacts, quiz.quizName, quiz.quizAuthor, quiz.quizSubject, quiz.quizCodes);
			res.status(200).send();
		})
		.catch((err) => {
			console.log(err);
...
const emailInvites = (quizInvites, quizName, quizAuthor, quizSubject, quizCodes) => {
	quizInvites.forEach((contact, index) => {
		...
		const mailOptions = {
			...
			html: `<h1>You've been invited to take a quiz!</h1><br><p><strong>Name: </strong>${quizName}</p><br><p><strong>Subject: </strong>${quizSubject}</p><br><p><strong>Author: </strong>${quizAuthor}</p><br><p>Log in with code: ${quizCodes[
				index
			].code}</p><br><a href="#">Go to Quiz Master</a>`
		};
...
app.post("/deleteContact", auth, (req, res) => {
	...
	User.findById(id)
		.then((user) => {
			const updatedContacts = user.contacts.filter((contact) => contact._id.toString() !== contactId);
...
app.post("/deleteGroup", auth, (req, res) => {
	...
	User.findById(id)
		.then((user) => {
			const updatedGroups = user.groups.filter((group) => group._id.toString() !== groupId);
		...
		.then((user) => {
			const updatedGroups = user.groups.map((group) => {
				if (group._id.toString() === groupId) {
					group.contacts = [ ...group.contacts.filter((contact) => contact._id.toString() !== memberId) ];
				}
				return group;
			});
			user.save().then(() => {
