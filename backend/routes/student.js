    ...
    quizzes.forEach((quiz) => {
      quiz.quizInvites.contacts.forEach((contact) => {
        if (contact.code === studentCode) {
          //quiz exists
          const scoreFromDb = quiz.quizScores.find(
            (score) => score.studentId === contact.id
          );
          if (scoreFromDb) {
            ...
            if (
              ...
              parseInt(quiz.quizTimeLimit) * 60000
            ) {
          ...
          res.status(200).send({
            quiz: {
              ...
              quizQuestions: quiz.quizQuestions.slice(
...
router.post("/saveScore", checkAuth, async (req, res) => {
  ...
  try {
    const quiz = await Quiz.findById(quizId);
    const updatedRecord = quiz.quizScores.find(
...
router.post("/finishQuiz", async (req, res) => {
  ...
  try {
    ...
    await quiz
      .save()
      .then(() => res.status(200).send({ msg: "quiz submitted" }))
